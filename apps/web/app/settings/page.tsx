...
export default function SettingsPage() {
  ...
  return <div className="page inner-page settings-page">
    ...
    {activeBridge && <section className="settings-section admin-card runtime-control"><div className="runtime-heading"><div><span className="eyebrow">ANIME EXECUTION</span><h2>Isolated anime host</h2><p>Compatible Aniyomi APKs run in a constrained local JVM with signed-package checks, network allowlisting, and process limits.</p></div><ShieldCheck/></div><div className="runtime-list">{bridgeRuntimes.filter((runtime)=>runtime.kind === "ANIME").map((runtime)=><div className="health-row" key={runtime.id}><span className={runtime.available ? "health-dot good" : "health-dot"}/><div><b>{runtime.id === "aniyomi-fixture-host" ? "Contained anime host" : "Aniyomi APK compatibility"}</b><small>{runtime.message}</small></div><span>{runtime.available ? "ready" : "unavailable"}</span></div>)}</div></section>}
