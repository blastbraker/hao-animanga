...
export default function SettingsPage() {
  ...
  useEffect(() => {
    void loadCloudState().catch((error: Error) => setMessage(error.message));
  }, []);
  useEffect(() => {
  ...
  async function removeExtension(item: InstalledExtension) {
    if (pendingRemoval !== item.packageName) {
      setPendingRemoval(item.packageName);
      return;
    }
    setBusyAction(item.packageName);
    try {
  ...
  return (
    <div className="page inner-page settings-page">
      ...
      {activeBridge?.scope === "beta" ? (
        <section className="settings-section bridge-panel beta-bridge-panel">
          <div>
          ...
          <div className="bridge-visual">
            ...
            <small>Managed Beta Bridge · no setup required</small>
          </div>
        </section>
      ) : (
        <section className="settings-section bridge-panel">
          <div>
      ...
      {activeBridge?.scope === "personal" && (
        <section className="settings-section admin-card runtime-control">
      ...
      {activeBridge?.scope !== "beta" && (
        <section className="settings-section">
          ...
          {repositories.length > 0 && (
            <div className="admin-card repository-list">
          ...
          {preview && (
            <div className="admin-card extension-browser">
              ...
              <div className="extension-list">
                {visiblePackages.map((item) => (
                  <div className="extension-row" key={item.pkg}>
                    <div>
                      <b>{item.name}</b>
                      <small>{item.pkg}</small>
                    </div>
          ...
          {inspection && (
            <div className="admin-card inspection-card">
              ...
              <dl className="inspection-facts">
                ...
                <div>
                  ...
                  <dd>
                    <code>{inspection.signerFingerprint}</code>
                  </dd>
                </div>
              </dl>
...
function Source({ icon, title, copy, action, active }: { icon: React.ReactNode; title: string; copy: string; action: string; active?: boolean }) {
  return (
    <article className="source-card">
      ...
      <button className={active ? "connected" : ""}>
        {active ? <Check /> : <Plus />}
        {action}
      </button>
    </article>
...
function packageMaturity(item: ExtensionPackage) {
