...
import { bridgeJson, getActiveBridge } from "../../lib/api";
...
export default function ActivityPage() {
    ...
    void getActiveBridge()
      .then(async ({ endpoint }) => {
        const [anime, manga] = await Promise.allSettled([
          bridgeJson<Array<{ id: string; name: string }>>(endpoint, "/v1/anime/sources"),
          bridgeJson<Array<{ id: string; name?: string; displayName?: string }>>(endpoint, "/v1/manga/sources")
        ]);
        if (cancelled) return;
        const names: Record<string, string> = {};
        if (anime.status === "fulfilled") anime.value.forEach((source) => { names[`anime:${source.id}`] = source.name; });
        if (manga.status === "fulfilled") manga.value.forEach((source) => { names[`manga:${source.id}`] = source.displayName ?? source.name ?? "Manga source"; });
  ...
  return <div className="page inner-page activity-page">
    ...
    <div className="activity-columns">
      <section className="activity-section"><div className="section-head"><div><span className="eyebrow">SMART FAILOVER</span><h2>Source health</h2></div><Gauge /></div>{sources.length ? <div className="health-list">{sources.map(([key, source]) => <div className="source-health" key={key}><span className={source.consecutiveFailures ? "health-dot" : "health-dot good"}/><span><b>{sourceLabel(key, sourceNames, items)}</b><small>{source.successes} successful · {source.failures} failed · {Math.round(source.averageLatencyMs)} ms</small></span><strong>{reliabilityScore(source) >= 30 ? "Healthy" : source.consecutiveFailures ? "Degraded" : "Learning"}</strong></div>)}</div> : <Empty icon={<Gauge />} title="Health checks are learning" copy="HAO ranks sources automatically as you browse and play." />}</section>
...
function sourceLabel(key: string, names: Record<string, string>, items: ActivityEntry[]) {
  const [kind = "source", id = ""] = key.split(":");
  const activityName = items.find((item) => {
    try { return new URL(item.href, "https://hao.local").searchParams.get("sourceId") === id; }
    catch { return false; }
  })?.sourceName;
  const type = kind === "anime" ? "Anime" : kind === "manga" ? "Manga" : "Source";
  return `${type} · ${names[key] ?? activityName ?? `Source ${id.slice(-6)}`}`;
}
