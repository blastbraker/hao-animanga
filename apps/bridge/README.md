...
The Aniyomi APK compatibility runtime remains disabled. Repository installation alone never authorizes arbitrary anime APK execution. The separate-process protocol, authentication, resource cap, crash recovery, and fixture conformance gates now pass. Before third-party APKs can load, the host still needs OS-enforced filesystem/network sandboxing (Windows AppContainer or Job Object policy, macOS sandbox profile, and Linux namespaces/seccomp) plus permission-delta and malicious-fixture tests.
