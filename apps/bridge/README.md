...
The Aniyomi APK compatibility runtime remains execution-disabled. Repository installation alone never authorizes arbitrary anime APK execution. Inside the isolated host, enabled anime APKs now undergo a second hash and Android-signature check, manifest/source-class validation, checksum-pinned Dex-to-JVM translation, and linkage against the official Aniyomi v14 API. The local endpoint `GET /v1/anime/extensions/probe` reports those results; it never constructs a source class or performs extension network requests. The installed AniWave 14.3 APK passes this admission/linkage stage.

Before source constructors and catalog calls are enabled, the host still needs a concrete Aniyomi v14 runtime implementation (the published extension library is compile-time API stubs), OS-enforced filesystem/network sandboxing (Windows AppContainer or Job Object policy, macOS sandbox profile, and Linux namespaces/seccomp), plus signed fixture, permission-delta, malicious-fixture, crash, and outbound-policy tests.
